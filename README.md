...
Для локальной демонстрации работы модели реализовано Streamlit-приложение `app.py`.

Приложение позволяет пользователю ввести параметры заказа, товара и доставки, после чего загружает сохранённую модель `models/best_model.joblib` и выводит прогноз целевой переменной `payment_value`.
...
```
Демонстрация работы Streamlit-приложения: [ссылка на видео](https://disk.yandex.ru/d/_wU-As6XxRawYg)
